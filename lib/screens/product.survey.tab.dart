...
class ProductSurveyTab extends StatefulWidget {
  final String productId;

  // final Survey survey;
  // final Function(bool value, int totaEarning) submit;

  const ProductSurveyTab({
    super.key,
  ...
  @override
  State<ProductSurveyTab> createState() => _ProductSurveyTabState();
}

class _ProductSurveyTabState extends State<ProductSurveyTab> {
  ...
  saveSurvey(AppProvider provider) async {
    ...
    try {
      ...
      if (response != null && response.data['status']) {
        if (!mounted) return;
        context.router.popUntilRouteWithName(BrandItemRoute.name);
        if (!mounted) return;
